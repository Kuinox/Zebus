...
public abstract class MessageHandlerInvoker : IMessageHandlerInvoker
{
    ...
    protected MessageHandlerInvoker(IServiceProvider serviceProvider, Type handlerType, Type messageType, MessageHandlerInvokerSubscriber subscriber)
    {
    ...
    protected internal object CreateHandler(MessageContext messageContext)
    {
        if (IsHandlerSingleton())
            return _serviceProvider.GetRequiredService(MessageHandlerType);
        ...
        try
        {
            ...
            return _serviceProvider.GetRequiredService(MessageHandlerType, _instance);
        }
        finally
        {
            _dispatchBus = null;
